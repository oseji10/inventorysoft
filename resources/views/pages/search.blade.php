					...
					<form action="{{ route('search') }}" method="GET">
						...
						<select class="js-example-basic-single col-sm-12" name="search">
							<optgroup label="Coordinators">
								{{$coordinators =  App\Models\User::select('first_name', 'last_name', 'id')->where('role_id', '1')->get();}}
								@forelse($coordinators as $item)
								<option value="{{$item->id}}" >{{$item->first_name}} {{$item->last_name}} {{$item->id}}</option>
								@empty
								@endforelse
							</optgroup>
						</select><br/>
					...
					<div class="dt-ext table-responsive">
						<table class="display" id="export-button" >
							...
							<tbody>
								@forelse ($query as $q)
								<tr>
									<td>{{$q->first_name ?? null}} {{$q->last_name ?? null}} {{$q->other_names ?? null}}</td>
									<td>{{$q->bvn ?? null}}</td>
									<td>{{$q->nin ?? null}}</td>
									<td>{{$q->phone_number ?? null}}</td>
